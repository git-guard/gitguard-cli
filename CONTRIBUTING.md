...
- Or run the local build without linking: `GITGUARD_API_URL=http://localhost:3100 yarn scan` (from this directory)
...
gitguard --version
```
If you see an old version (e.g. 1.3.1) or a path under `nvm`/`npm`, the npm global is taking precedence. Uninstall it so the yarn-linked CLI is used:
```bash
...
**"There's already a package called @gitguard/cli registered"** — Yarn is still pointing at another folder. Clear the old link, then link this folder:

```bash
# See what’s currently linked (optional)
ls -la ~/.config/yarn/link/

# Remove the scoped package link (@gitguard/cli lives under @gitguard/)
rm -rf ~/.config/yarn/link/@gitguard

# Then link this folder
yarn link
```
